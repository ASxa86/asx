...
namespace asx
{
	...
	///
	template <typename T>
	class PropertyTemplateMethods : public Property
	{
	public:
		...
		PropertyTemplateMethods(std::string_view n, Writer w, Reader r) : Property{ n }, writer{ std::move(w) }, reader{ std::move(r) }
		{
		}
		...
		{
			this->writer(asx::FromString<T>(x));
		}
