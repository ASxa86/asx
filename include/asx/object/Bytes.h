...
namespace asx
{
	...
	auto ToBytes([[maybe_unused]] const T& x)
	{
		...
		else if constexpr(asx::is_array<TNoRef>::value == true || std::is_same<TNoRef, std::string>::value == true
						  || std::is_same<TNoRef, std::string_view>::value == true || asx::is_set<TNoRef>::value == true
						  || asx::is_unordered_set<TNoRef>::value == true || asx::is_deque<TNoRef>::value == true
						  || asx::is_vector<TNoRef>::value == true)
		{
		...
		else if constexpr(asx::is_pair<TNoRef>::value == true)
		{
		...
		else if constexpr(asx::is_map<TNoRef>::value == true || asx::is_unordered_map<TNoRef>::value == true)
		{
			auto b = ToBytes(x.size());
			v.insert(std::end(v), std::cbegin(b), std::cend(b));
	...
	std::size_t BytesTo([[maybe_unused]] std::span<std::byte> x, T& t)
	{
		...
		else if constexpr(asx::is_array<TNoRef>::value == true)
		{
			...
			return skip;
						  ...
						  || asx::is_vector<TNoRef>::value == true || asx::is_deque<TNoRef>::value == true)
		{
		...
		else if constexpr(asx::is_set<TNoRef>::value == true || asx::is_unordered_set<TNoRef>::value == true)
		{
			typename TNoRef::size_type s{};
			auto skip = BytesTo<decltype(s)>(x, s);
