...
namespace asx
{
	...
	std::string ToString([[maybe_unused]] const T& x)
	{
		...
		if constexpr(asx::is_duration<TNoRef>::value == true)
		{
		...
		else if constexpr(asx::is_pair<TNoRef>::value == true)
		{
			...
			if constexpr(asx::is_string<first_type>::value == true)
			{
				s += "\"";
			}
			...
			if constexpr(asx::is_string<second_type>::value == true)
			{
				s += "\"";
			}
		...
		else if constexpr(asx::is_container<TNoRef>::value == true || asx::is_array<TNoRef>::value == true)
		{
			std::string s = "[";
	...
	void StringTo([[maybe_unused]] std::string_view x, T& t)
	{
		...
		else if constexpr(asx::is_container<TNoRef>::value == true)
		{
			auto it = std::begin(x);
			const auto end = std::end(x);
	...
	typename std::remove_reference<T>::type StringTo(std::string_view x)
	{
		...
		if constexpr(asx::is_pair<TNoRef>::value == true)
		{
